...
##### Outputs
## Results: [*Intervention_project*OR*Mobility_data_project*]/
# Model_deaths/*sensitivity_analysis*/*output_filename_[interventionsORmobility_data]*.csv
...
if (mobility_data_sources == "yes") {
  ...
  if (sensitivity_analysis == "no_sensitivity") {
    if (dir.exists(paste0(directory_stem, "Mobility_data_project/Model_deaths"))) {
      paste0(directory_stem, "Mobility_data_project/Model_deaths/", output_filename_mobility_data, ".csv") %>%
        fwrite(Deaths, .)
    } else {
      dir.create(paste0(directory_stem, "Mobility_data_project/Model_deaths"))
      paste0(directory_stem, "Mobility_data_project/Model_deaths/", output_filename_mobility_data, ".csv") %>%
        fwrite(Deaths, .)
    }
  } else {
    if (dir.exists(paste0(directory_stem, "Mobility_data_project/Model_deaths/", sensitivity_analysis))) {
      paste0(directory_stem, "Mobility_data_project/Model_deaths/", sensitivity_analysis, "/", 
             output_filename_mobility_data, ".csv") %>%
        fwrite(Deaths, .)
    } else {
      dir.create(paste0(directory_stem, "Mobility_data_project/Model_deaths/", sensitivity_analysis))
      paste0(directory_stem, "Mobility_data_project/Model_deaths/", sensitivity_analysis, "/", 
             output_filename_mobility_data, ".csv") %>%
        fwrite(Deaths, .)
    }
