...
##### Author: Jordan Klein
# * = to supply via shell
#### Two projects: *intervention_scenarios* & *mobility_data_sources*
##### Inputs
#### Files
### Fixed
...
### Mobility network
# *network_data_source* (Commuter_survey = Mobility/Commuting_matrix.csv, InLoco = Mobility/network_processed.zip)
### Interventions
## Social distancing
# *social_distancing_intervention* (no, actual, equality)
# *social_distancing_metric* (abs_delta, pr_stayhome)
# *social_distancing_data_source* (Facebook, InLoco)
# abs_delta: Mobility/mvt_range_maps/Mobility_change_*social_distancing_intervention*.csv
# pr_stayhome & Facebook: Mobility/mvt_range_maps/prob_single_tile.csv
# pr_stayhome & InLoco: Mobility/social_distancing_imputed.csv
## Vaccination
...
#### Parameters
### Fixed
# Latent period = 2.9 days (Syria paper)
# Infectious period = 7 days (Syria paper)
...
## sd_0 
# .300 (baseline social distancing from peixoto, feb 2020)
# .199 (baseline social distancing from Facebook movement range maps, 1st wk of Mar 2020)
### Variable (sensitivity analyses)
# *R0* (de Souza paper) = 3.1 [2.4, 5.5]
# *SAR* (household SAR, Madewell & Thompson papers) = .189 [.14, .248]
##### Output
## Results: Model_output/[*Intervention_project*OR*Mobility_data_project*]/
# *sensitivity_analysis*/*output_filename_[interventionsORmobility_data]*.csv
...
} else if (network_data_source == "Commuter_survey") {
  ## Commuter survey data (single matrix)
  Mob_network <- fread("Data/Mobility/commuting_survey/Commuting_matrix.csv") %>%
    .[order(.$muni_code), ] %>%
...
if (social_distancing_intervention != "no") {
  ...
  } else if (social_distancing_metric == "pr_stayhome") {
    # Probability of staying home (w/in tile)
    # *supply social_distancing_data_source via shell*
    if (social_distancing_data_source == "Facebook") {
      # Facebook
      SD <- fread("Data/Mobility/mvt_range_maps/prob_single_tile.csv")
    } else if (social_distancing_data_source == "InLoco") {
      # InLoco
      SD <- fread("Data/Mobility/social_distancing_imputed.csv")
    }
    # Adjust for sensitivity analysis *supply NPI_sensitivity via shell*
    SD <- mutate(SD, sd = sd * NPI_sensitivity) %>%
      mutate(sd = if_else(sd < 0, 0, sd),
             sd = if_else(sd > 1, 1, sd)) %>%
      # Split into list by week
      split(.$week) %>%
      lapply(function(x) {
        x[order(x$muni_code), c("muni_code", "sd")]
      }) 
  }
}
...
# Known parameters
delta_E <- 2.9^-1 # Latent period = 2.9 days (Syria paper)
gamma <- 7^-1 # Infectious period = 7 days (Syria paper)
...
# Baseline social distancing (only used when *social_distancing_metric* = "pr_stayhome")
if (social_distancing_metric == "pr_stayhome") {
  if (social_distancing_data_source == "Facebook") {
    sd_0_bar <- .199 # mean sd_0 (baseline social distancing from Facebook movement range maps, 1st wk of Mar 2020)
  } else if (social_distancing_data_source == "InLoco") {
    sd_0_bar <- .300 # mean sd_0 (baseline social distancing from Peixoto, Feb 2020)
  }
}
  ...
  params <- list(
    ...
    NPI = if (social_distancing_intervention == "no") {
      ...
      if (social_distancing_metric == "abs_delta") {
        ...
        (1 - rep(SD, each = 7)[[i]]$sd) / (1 - sd_0_bar) # Probability of staying home
      }
    },
    # Vaccination
...
if (mobility_data_sources == "yes") {
  ...
  # *supply sensitivity_analysis, output_filename_mobility_data via shell*
  if (sensitivity_analysis == "no_sensitivity") {
    paste0(proj_directory, output_filename_mobility_data, ".csv") %>%
      fwrite(Model_format, .)
  } else {
    if (dir.exists(paste0(proj_directory, sensitivity_analysis))) {
      paste0(proj_directory, sensitivity_analysis, "/", output_filename_mobility_data, ".csv") %>%
        fwrite(Model_format, .)
    } else {
      dir.create(paste0(proj_directory, sensitivity_analysis))
      paste0(proj_directory, sensitivity_analysis, "/", output_filename_mobility_data, ".csv") %>%
